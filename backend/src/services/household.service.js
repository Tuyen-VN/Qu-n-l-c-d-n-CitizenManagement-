...
class HouseholdService {
    ...
    try {
      ...
      const dataQuery = `
        SELECT
          ...
          c.permanent_address as head_permanent_address,
          w.ward_name,
          d.district_name,
          p.province_name,
          h.created_at
  ...
  async createHousehold(householdData, createdBy) {
    ...
    try {
      ...
      // Dong bo dia chi thuong tru cho chu ho
  ...
  async updateHousehold(householdId, householdData) {
    try {
      ...
      // Neu cap nhat address, dong bo permanent_address cho tat ca thanh vien hien tai
      if (householdData.address) {
        await pool
          .request()
          .input('householdId', sql.Int, householdId)
          ...
          .query(`
            UPDATE c
            SET c.permanent_address = @address,
  ...
  async addHouseholdMember(householdId, memberData) {
    try {
      ...
      // Kiem tra bản ghi cu (da tung la thanh vien, hom nay bi xoa roi them lai -> vi pham UNIQUE)
      const existingRecord = await pool
        .request()
        .input('household_id', sql.Int, householdId)
        .input('citizen_id', sql.Int, memberData.citizen_id)
        .query(`
          SELECT member_id, is_current_member
          FROM HouseholdMembers
      ...
      if (existingRecord.recordset.length > 0) {
        // Ban ghi cung ngay da co (bi xoa roi them lai trong ngay) -> UPDATE lai
        await pool
          .request()
          .input('household_id', sql.Int, householdId)
          .input('citizen_id', sql.Int, memberData.citizen_id)
        ...
        // Chua co ban ghi nao hom nay -> INSERT moi binh thuong
        await pool
          .request()
          .input('household_id', sql.Int, householdId)
          .input('citizen_id', sql.Int, memberData.citizen_id)
      ...
      // Dong bo permanent_address cho thanh vien moi them
      if (household.address) {
        await pool
          .request()
          .input('citizenId', sql.Int, memberData.citizen_id)
          .input('address', sql.NVarChar, household.address)
          .input('wardId', sql.Int, household.ward_id)
          .query(`
            UPDATE Citizens
            SET permanent_address = @address,
                ward_id = @wardId,
      ...
      logger.info(`Member added/restored to household ${householdId}`);
